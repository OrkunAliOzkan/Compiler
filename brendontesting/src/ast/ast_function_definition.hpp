    ...
    virtual std::string Compile( int &mem, std::map<std::string, std::pair<std::string, double>> g_Var, std::map<std::string, std::pair<std::string, double>> loc_Var, std::string type_check , bool &isConstant) override
    {
        if(expr4 == NULL)
        {
            ...
            else 
            {
                ...
                {
                    mem = 4;
                    std::string contents = expr3->Compile(mem, g_Var, loc_Var, type_check, isConstant);   //  contents of foo

                    int mem_used;
                    if (mem == 4)   { mem_used = -8; }
                    else            { mem_used = -24 - 8*(int( mem / 8 ) - 1); }

                    std::string func_name = expr2->Compile(mem, g_Var, loc_Var, type_check, isConstant);
                    ...
                    std::string ending;
                    std::string type_specifiers (expr1->Compile(mem, g_Var, loc_Var, type_check , isConstant));
